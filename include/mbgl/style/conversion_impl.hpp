...
/**
   ...
   a filled std::optional is returned.
   ...
   specialization must provide the following static methods:
      ...
      * `toBool(v)` -- returns `std::optional<bool>`, absence indicating `v` is not a JSON boolean
      * `toNumber(v)` -- returns `std::optional<float>`, absence indicating `v` is not a JSON number
      * `toDouble(v)` -- returns `std::optional<double>`, absence indicating `v` is not a JSON number
      * `toString(v)` -- returns `std::optional<std::string>`, absence indicating `v` is not a JSON string
      * `toValue(v)` -- returns `std::optional<Value>`, a variant type, for generic conversion,
