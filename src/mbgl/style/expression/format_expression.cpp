...
ParseResult FormatExpression::parse(const Convertible& value, ParsingContext& ctx) {
    ...
    for (std::size_t i = 1; i <= argsLength - 1; ++i) {
        ...
        if (nextTokenMayBeObject && isObject(arg)) {
            ...
            const std::optional<Convertible> fontScaleOption = objectMember(arg, kFormattedSectionFontScale);
            ...
            const std::optional<Convertible> textFontOption = objectMember(arg, kFormattedSectionTextFont);
            ...
            const std::optional<Convertible> textColorOption = objectMember(arg, kFormattedSectionTextColor);
