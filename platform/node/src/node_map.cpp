...
void NodeMap::QueryRenderedFeatures(const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    try {
        std::vector<mbgl::Feature> std::optional;
        ...
        if (Nan::Get(posOrBox, 0).ToLocalChecked()->IsArray()) {
            ...
            std::optional = nodeMap->frontend->getRenderer()->queryRenderedFeatures(mbgl::ScreenBox {
                {
            ...
            std::optional = nodeMap->frontend->getRenderer()->queryRenderedFeatures(mbgl::ScreenCoordinate {
        ...
        for (std::size_t i = 0; i < std::optional.size(); i++) {
            Nan::Set(array, static_cast<uint32_t>(i), toJS(std::optional[i]));
        }
