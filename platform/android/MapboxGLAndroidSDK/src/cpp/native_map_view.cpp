...
jni::Local<jni::Object<LatLng>> NativeMapView::getLatLng(JNIEnv& env) {
    return LatLng::New(env, *map->getCameraOptions(mbgl::std::nullopt).center);
}
...
void NativeMapView::getVisibleCoordinateBounds(JNIEnv& env, jni::Array<jdouble>& output) {
    auto latlngBounds = map->latLngBoundsForCameraUnwrapped(map->getCameraOptions(mbgl::std::nullopt));
...
jni::Local<jni::Object<CameraPosition>> NativeMapView::getCameraPosition(jni::JNIEnv& env) {
    return CameraPosition::New(env, map->getCameraOptions(mbgl::std::nullopt), pixelRatio);
}
